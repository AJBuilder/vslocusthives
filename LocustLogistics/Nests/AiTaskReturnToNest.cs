...
namespace LocustLogistics.Nests
{
    public class AiTaskReturnToNest : AiTaskBase, IAiTask
    {
        ...
        LocustNestModSystem nestsModSystem;

        ILocustNest targetNest;
        IReadOnlyDictionary<ILocustNest, int> nestMembership;
        ...
        public AiTaskReturnToNest(EntityAgent entity, JsonObject taskConfig, JsonObject aiConfig) : base(entity, taskConfig, aiConfig)
        {
            ...
            nestsModSystem = entity.Api.ModLoader.GetModSystem<LocustNestModSystem>();
            membership = entity.Api.ModLoader.GetModSystem<LocustHivesModSystem>().Membership;
            nestMembership = nestsModSystem.Membership;
        }
        ...
        public override bool ShouldExecute()
        {
            if (member == null ||
                ...
                !membership.TryGetValue(member, out var hive)) return false;

            // Find nearest nest with room
            ILocustNest nearest = null;
            double minDistSq = double.MaxValue;

            foreach (var nest in nestsModSystem.GetHiveNests(hive))
            {
